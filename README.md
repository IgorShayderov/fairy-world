...
Low-level ratings use denominator floors: crit/dodge chance is `min(50, rating × 65 / max(level, 50))`. Critical damage starts at 150%: with `effective = rating × 15 / max(level, 100)`, its percentage is `150 + 150 × effective / (effective + 6)`, capped at 300%. Defense uses `max(level, 10)`. With the five starting attributes and no gear/buffs this gives 3.3% crit, 3.3% dodge, 4.8% defense, and 166.7% critical damage.
...
Travel has a 40% encounter chance per step. Travel victory loot odds are 80% no drop, 13.5% common, 4.5% magic, 1.5% rare, and 0.5% unique. Death returns the player to Evercross and removes all active buffs; sanctuary cooldowns remain intact. Icevault in the north and Ravencrypt in the northwest use the same dungeon rules.
