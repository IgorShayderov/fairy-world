...
export function generateTownOffers(town: { shopId: number; x: number; y: number }, now: Date) {
  const day = now.toISOString().slice(0, 10);
  const expiresAt = new Date(`${day}T00:00:00Z`);
  expiresAt.setUTCDate(expiresAt.getUTCDate() + 1);
  let roll = 0;
  const random = () =>
    createHash('sha256').update(`quest-board-v1:${town.shopId}:${day}:${roll++}`).digest().readUInt32BE(0) /
    0x100000000;
  ...
  return Array.from({ length: 3 }, (_, slot) => {
    ...
    return {
      code: `town_${town.shopId}_${day}_${slot}`,
      townId: town.shopId,
