<template>
  <div
    class="relative z-20 w-full shrink-0 overflow-visible transition-[height] duration-500 ease-in-out"
    :class="chatAnchorClasses"
  >
    <article
      class="realm-chat absolute bottom-0 left-0 w-full overflow-visible bg-[#071923] text-slate-200 transition-[height] duration-500 ease-in-out"
    ...
    >
      ...
      <div class="flex h-full w-full overflow-hidden">
        ...
        <section class="flex h-full w-4/5 flex-col">
          ...
          <div
            class="flex-1 space-y-3 overflow-y-auto bg-[radial-gradient(circle_at_top,#12313d_0,#081b26_58%,#061620_100%)] p-4"
...
const chatAnchorClasses = computed(() => {
  return {
    'h-[300px]': chatState.chatPosition === 'standard',
    'h-0': chatState.chatPosition !== 'standard',
  };
});
const chatPanelClasses = computed(() => {
  return {
    'h-[calc(100vh-var(--header-height))] border-t border-[#806f43] shadow-[0_-20px_50px_rgba(0,0,0,0.45)]':
      chatState.chatPosition === 'full-screen',
...
const dateLabel = (group: MessageDateGroup) =>
  group.isToday
    ? t('chat.labels.today')
