  ...
  >
    ...
    <div class="flex flex-wrap gap-2">
      <button
        ...
        :class="[compact ? 'h-9 w-9 p-1' : 'h-16 w-16 p-2', buff.value < 0 ? 'border-red-500/80' : colors[buff.type]]"
        :aria-label="`${buffTitle(buff)}: ${effect(buff)}. ${remaining(buff.expiresAt)}`"
      ...
      >
        ...
        >
          <div class="font-bold" :class="buff.value < 0 ? 'text-red-400' : ''">{{ buffTitle(buff) }}</div>
...
const buffTitle = (buff: ActiveBuff) =>
  buff.value < 0 ? t(`profile.curseNames.${buff.type}`) : t(`profile.buffNames.${buff.type}`);
const effect = (buff: ActiveBuff) =>
  buff.value < 0
    ? t(`profile.curseDescriptions.${buff.type}`, { value: Math.abs(buff.value) })
    : t(`profile.buffDescriptions.${buff.type}`, { value: buff.value });
