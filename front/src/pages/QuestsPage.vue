          ...
          <div class="grid gap-4 md:grid-cols-2">
            <article v-for="entry in journal.active" :key="entry.questId" class="rounded-xl border border-blue-100 bg-white p-5 shadow-sm">
              ...
              <template v-if="!entry.quest.isPrimary">
                <button v-if="entry.quest.destinationTownId" class="mt-3 block text-sm text-blue-600 underline" :disabled="pending !== null || journal.town?.id !== entry.quest.destinationTownId" @click="deliver(entry.questId)">{{ t('quests.deliver') }}</button>
                ...
                <div v-else class="mt-4 rounded-lg bg-red-50 p-3 text-sm">
                  ...
                  <div class="mt-3 flex gap-4">
                    <button class="text-red-700 underline" :disabled="pending !== null" @click="cancel(entry.questId)">{{ t('quests.confirmCancel') }}</button>
                    <button class="underline" :disabled="pending !== null" @click="cancelingId = null">{{ t('quests.keep') }}</button>
                  </div>
                </div>
              </template>
            </article>
...
const deliver = async (id: number) => {
  ...
  try {
    ...
    await load();
  } catch { error.value = t('quests.deliveryError'); }
...
const cancel = async (id: number) => {
  ...
  try {
    ...
    await load();
  } catch { error.value = t('quests.cancelError'); }
  finally { pending.value = null; }
};
</script>
